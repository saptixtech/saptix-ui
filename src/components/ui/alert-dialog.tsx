  ...
  return (
    <AlertDialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          'bg-popover text-popover-foreground data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 fixed top-1/2 left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl border border-[var(--border)] p-6 shadow-2xl duration-150 outline-none sm:max-w-lg',
          className
        )}
        {...props}
      >
...
function AlertDialogAction({
  ...
  variant = 'default',
  ...props
}: DialogPrimitive.Close.Props & { variant?: 'default' | 'destructive' | 'outline' | 'secondary' }) {
  return (
    <DialogPrimitive.Close
      data-slot="alert-dialog-action"
      render={<Button variant={variant} className={className as any} />}
      {...props}
    />
  )
}
...
function AlertDialogCancel({
  ...
  return (
    <DialogPrimitive.Close
      ...
      render={<Button variant="outline" className={className as any} />}
      {...props}
    />
  )
}
